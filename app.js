...
const FocusView = {
    ...
    startFocus(task) {
        ...
        // ===== 反逃逸：阻止浏览器后退按钮 =====
        // 推入一个虚拟历史记录，后退时会触发 popstate 而不是离开
        history.pushState({ focusLock: true }, '', window.location.href);
        this._popstateHandler = (e) => {
            // 每次后退都重新推入，让后退按钮失效
            history.pushState({ focusLock: true }, '', window.location.href);
            toast('专注模式中，禁止后退！长按底部按钮紧急退出', 'warning');
        };
    ...
    _cleanupEscapeBlock() {
        // 移除 popstate 监听，恢复正常的浏览器后退
        if (this._popstateHandler) {
            window.removeEventListener('popstate', this._popstateHandler);
            this._popstateHandler = null;
        }
        // 清除我们推入的虚拟历史记录
        if (history.state && history.state.focusLock) {
            history.back();
        }
    },
